...
DICCIONARIO_CHOFERES = {
    "1234-ABC": "Juan Pérez",
    "5678-XYZ": "Carlos López",
    "9012-DEF": "Luis Martínez"
}

# Precios fijos para los cálculos matemáticos
COSTO_LLANTA_POR_KM = 0.50
COSTO_ACEITE_POR_KM = 0.20
PRECIO_DIESEL_POR_LITRO = 3.74
        ...
        try:
            ...
            if st.button("Guardar Registro de Viaje"):
                if not codigo_cfo:
                    ...
                    # Guardado de datos optimizado combinando DataFrames
                    nueva_fila = pd.DataFrame([nuevo_registro])
                    df_actualizado = pd.concat([df_existente, nueva_fila], ignore_index=True)
                    conn.update(data=df_actualizado)
                    
                    st.balloons()
                    st.success("✅ ¡Viaje guardado! Flete registrado correctamente en la base de datos.")
